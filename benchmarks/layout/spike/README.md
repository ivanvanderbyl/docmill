...
| path | what |
|---|---|
| `cmd/spike/` | Go: emitter, `leaves` predictor, codegen predictor, fixture check. `featureNames` in `features.go` is the feature contract. |
| `cmd/spike/layoutmodel.txt` | trained artefact, `go:embed`ed |
| `cmd/spike/layoutmodel_gen.go` | generated by `spike gen` — flat-array form of the same trees |
...
`spike gen` regenerates `layoutmodel_gen.go` from the embedded artefact, and
`spike explain <pdf>` prints the decision path behind the highest-scoring lines.

**Bootstrap wrinkle:** the generator lives in the binary that consumes its output, so
after changing what `gen.go` emits, the package will not compile until a placeholder
`layoutmodel_gen.go` exists. Write a stub with the new declarations, build, run
`spike gen`, then rebuild.

`go test ./benchmarks/layout/spike/cmd/spike/` asserts the generated model and `leaves`
are the same function (bit-identical over 200k random vectors); `-bench 'Predict|Load'`
is the comparison behind the research note's codegen recommendation.
