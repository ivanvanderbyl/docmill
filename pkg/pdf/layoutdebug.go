...
// RegionDebugRow is one candidate region: its proposed class from the line
// model, its box, and its region-scoped feature vector.
type RegionDebugRow struct {
	Doc      string    `json:"doc"`
	Page     int       `json:"page"`
...
// LayoutRegionRows groups a document's lines into candidate regions and
// describes each one.
//
// The line labels come from the EMBEDDED model and the grouping and features
// from the shared Go path, which the plan requires explicitly: a Python
// reimplementation of either would reintroduce training/serving skew, and this
// stage consumes the previous stage's output rather than the raw page.
func LayoutRegionRows(ctx context.Context, doc Document, name string, options ExtractionOptions) ([]RegionDebugRow, error) {
	pageCount, err := doc.PageCount(ctx)
	if err != nil {
		return nil, err
	}

	var rows []RegionDebugRow
	for index := 0; index < pageCount; index++ {
		state, err := capturePageDebugState(ctx, doc, index, options)
		if err != nil {
			return nil, err
		...
		for _, region := range GroupLineRegions(state.lines, labeller.labels) {
			rows = append(rows, RegionDebugRow{
				Doc: name, Page: index + 1, Class: region.Class,
				PageW: state.size.Width, PageH: state.size.Height,
				L: region.Box.L, T: topEdgeOf(region.Box), R: region.Box.R, B: bottomEdgeOf(region.Box),
				Lines:    len(region.Lines),
				Features: RegionFeatures(region, state.lines, labeller.labels, cells, state.rulings, state.size),
			})
		}
	}
	return rows, nil
